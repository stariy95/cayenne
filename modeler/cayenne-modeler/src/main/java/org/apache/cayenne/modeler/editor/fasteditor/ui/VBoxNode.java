...
public class VBoxNode extends Node {
    ...
    protected void updateChildrenLayout() {
        ...
        for(Node node : children) {
            ...
            Rectangle2D rect = new Rectangle2D(oldRect.getMinX(), height, oldRect.getWidth(), oldRect.getHeight());
            node.setBoundingRect(rect);
            height += oldRect.getHeight() + padding;
            width = Math.max(width, oldRect.getWidth());
        }
