 ...
 * @since 4.2
 */
public class PermanentObjectIdVisitor implements DiffSnapshotVisitor<Void> {

    static final String DB_ID_PREFIX = "db:";
    ...
    public Void visitInsert(InsertDiffSnapshot diffSnapshot) {
        ...
        createPermanentId(diffSnapshot);
        return null;
    }

    private void createPermanentId(InsertDiffSnapshot diffSnapshot) {
        ObjectId id = diffSnapshot.getChangeId();
        ...
        for (DbAttribute dbAttr : lastDbEntity.getPrimaryKeys()) {
            ...
            if(lastObjEntity != null) {
                ...
                if (objAttr != null) {
                    Object value = lastDescriptor.getProperty(objAttr.getName()).readPropertyDirectly(diffSnapshot.getObject());
                    if (value != null) {
