 ...
 * @since 4.2
 */
class OptimisticLockUpdateQualifierBuilder implements PropertyVisitor {

    private UpdateSnapshotCreationHandler updateSnapshotCreationHandler;
    private final Persistent object;

    OptimisticLockUpdateQualifierBuilder(UpdateSnapshotCreationHandler updateSnapshotCreationHandler, Persistent object) {
        this.updateSnapshotCreationHandler = updateSnapshotCreationHandler;
        this.object = object;
    }
    ...
    public boolean visitAttribute(AttributeProperty property) {
        if (property.getAttribute().isUsedForLocking()) {
            ...
            UpdateDiffSnapshot snapshot = updateSnapshotCreationHandler.getSnapshot(attribute);
    ...
    public boolean visitToOne(ToOneProperty property) {
        if (property.getRelationship().isUsedForLocking()) {
            ...
            for (DbJoin join : sourceRelationship.getJoins()) {
                UpdateDiffSnapshot snapshot = updateSnapshotCreationHandler.getSnapshot(join.getSource());
