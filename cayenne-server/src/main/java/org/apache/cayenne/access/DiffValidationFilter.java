 ...
 * @since 4.0
 */
public class DiffValidationFilter implements DataChannelFilter {
    ...
    public GraphDiff onSync(ObjectContext originatingContext, GraphDiff changes, int syncType, DataChannelFilterChain filterChain) {
        if((syncType == DataChannel.FLUSH_CASCADE_SYNC || syncType == DataChannel.FLUSH_NOCASCADE_SYNC)
                && originatingContext.isValidatingObjectsOnCommit()) {
            if(changes instanceof ObjectStoreGraphDiff && ((ObjectStoreGraphDiff)changes).validateAndCheckNoop()) {
                return changes;
            }
        }
