...
class PermObjectIdVisitor implements OperationVisitor<Void> {
    ...
    public Void visitInsert(InsertOperation operation) {
        ...
        // TODO: this method will gain some performance boost once operations are sorted by entity type
    ...
    private void createPermanentId(InsertOperation operation) {
        ...
        for (DbAttribute dbAttr : dbEntity.getPrimaryKeys()) {
            ...
            if (objAttr != null) {
                Object value = lastDescriptor.getProperty(objAttr.getName()).readPropertyDirectly(operation.getObject());
                if (value != null) {
