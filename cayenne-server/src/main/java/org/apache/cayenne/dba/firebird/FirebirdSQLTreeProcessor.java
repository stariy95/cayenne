...
public class FirebirdSQLTreeProcessor extends BaseSQLTreeProcessor {
    ...
    protected void onValueNode(Node parent, ValueNode child, int index) {
        replaceChild(parent, index, new ValueNode(child.getValue(), child.isArray(), child.getAttribute(), child.getJavaType()) {
            @Override
    ...
    private InNode newSliceNode(InNode child, Node arg, ValueNode valueNode, Object slice) {
        ...
        nextNode.addChild(new ValueNode(slice, valueNode.isArray(), valueNode.getAttribute(), valueNode.getJavaType()));
        return nextNode;
    }
